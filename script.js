var dataItems= [
  ...
  itemsWrapper.setAttribute('id', 'items');
  ...
  document.body.append(itemsWrapper,pagination);
  
  let current_page=1;
  let rows=10;

  function displayItems(dataItems,wrapper,rows,page){
      wrapper.innerHTML ="";
      ...
      let start=rows*page;
      let end=start+rows;
      let displayItems =dataItems.slice(start,end);
      for(var i=0;i<displayItems.length;i++){
          let item=displayItems[i];
          item_element=document.createElement('tr');
          item_element.classList.add('item')
          item_element.innerHTML = "<td>"+item.id+"</td>"+"<td>"+ item.name+"</td>" +"<td>"+ item.email+"</td>";
          tbody.appendChild(item_element);
      }
  }


    function setUpButtons(dataItems,wrapper,rows){
      wrapper.innerHTML="";
      totalPages=Math.ceil(dataItems.length/rows);
      let firstbtn=first();
      wrapper.appendChild(firstbtn);
      let prevbtn=previous();
      wrapper.appendChild(prevbtn);
      for(i=1;i<=totalPages;i++){
          let btn=pageButtons(i,dataItems);  
    ...
    function pageButtons(page,dataItems){
      ...
      button.addEventListener('click',()=>{
          current_page=page;
          displayItems(dataItems,itemsWrapper,rows,current_page)
  ...
  displayItems(dataItems,itemsWrapper,rows,current_page);
  setUpButtons(dataItems,pagination,rows);
